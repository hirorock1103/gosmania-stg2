<div class="table" style="margin-bottom: 10px;">
	...
	<?php foreach($table_ary['html'] as $col => $table_info ) { ?>
		<?php if($table_info['type'] == 'text' || $table_info['type'] == 'number' ){ ?>
		<div class="tr">
			<div class="th"><?php echo $table_info['col_name'].$table_info['must']; ?></div>
			<div class="td-edit">
				<input type="<?php echo $table_info['type']; ?>" name="<?php echo $col; ?>" value="<?php echo isset($data[$col]) ? $data[$col] : ''; ?>" class="" placeholder="<?php echo $table_info['placeholder']; ?>">
			</div>
		</div>
